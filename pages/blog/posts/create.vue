...
const { data: categories } = useFetch<Category[]>(`${config.public.apiBase}/blog/categories`)
...
const onSubmit = async () => {
  ...
  try {
    ...
    toast.add({
      title: 'Помилка',
      description: 'Не вдалося створити пост',
      color: 'red'
    })
  } finally {
...
<template>
  <div class="relative w-full min-h-screen bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e] px-4 py-16 text-white">
    <div class="max-w-2xl mx-auto bg-white/5 p-6 rounded-xl shadow-xl space-y-6 backdrop-blur-md border border-white/10">
      <h1 class="text-3xl font-bold text-white text-center">Створити пост</h1>
